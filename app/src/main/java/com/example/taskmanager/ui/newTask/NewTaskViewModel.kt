...
class NewTaskViewModel @Inject constructor(private val repository: NewTaskRepository) :
    ...
    fun addMember(memberPosition: Int){
        ...
        _taskMemberList.value = (_taskMemberList.value?.plus(item)  ?: mutableListOf(item)) as MutableList<TaskMember>?
    }
    ...
    fun setCurrentProject(position: Int){
        _currentProject.value = (projects.value as NetworkResult.Success<List<Project>>).data[position]
    }

    fun setCurrentMember(position: Int){
        _currentMember.value = (members.value as NetworkResult.Success<List<TaskMember>>).data[position]
    }
